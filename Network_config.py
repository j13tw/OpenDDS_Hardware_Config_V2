...
#        print(self.cpu_temperature)
    ...
    def remove_cpu_load_short(self):
        command = "sudo sed -i '10c \#max-load-1 = " +  self.cpu_short_load  + "' /etc/watchdog.conf"
        status = os.system(command)
        if (status == 0): return "OK"
        else: return "ERROR"
...
#        os.system('sudo /etc/init.d/ntp stop >/dev/null 2>&1')
        ...
        try:
            ...
            os.system('sudo cp ./library/ntp.log /etc/network/ntp.log')
...
#        print(eth1)
    ...
    def eth1_dhcp(self):
        ...
        #os.system('sudo lsusb | grep "Realtek" | cut -c16,17,18 >/tmp/usb.txt')
